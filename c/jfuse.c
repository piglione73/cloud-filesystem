...
int jfuse_getattr(const char *path, struct stat *buf) {
	//Attach to java
	JNIEnv *env = java_attach();
	
	//Create data object to represent buf
	jbyteArray jbuf = (*env)->NewByteArray(env, sizeof(*buf));
	(*env)->SetByteArrayRegion(env, jbuf, 0, sizeof(*buf), (jbyte *)buf);

	//Call method
	int retval = java_getattr(env, jbuf);

	//Get data and destroy
	(*env)->GetByteArrayRegion(env, jbuf, 0, sizeof(*buf), (jbyte *)buf);
